package ru.chalovai.PR4.example4;
...
{
    // JPanel представляет собой контейнер, который может хранить группу компонентов
    JPanel[] pnl = new JPanel[12];

    public BorderExample()
    {
        // Класс GridLayout-это менеджер компоновки, который раскладывает компоненты
        // контейнера в прямоугольную сетку. Контейнер разделен на прямоугольники
        // одинакового размера, и в каждый прямоугольник помещается по одному компоненту.
        setLayout(new GridLayout(3,4));

        for(int i = 0 ; i < pnl.length ; i++)
        {
            // Закрашивание фона прямоугольников
